    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    RegisterAction<regthree_t, bit<1>, bit<1>>(regthree) midwindow = {
        void apply(inout regthree_t value, out bit<1> ret) {
    ...
    RegisterAction<regthree_t, bit<1>, int<32>>(regthree) firstwindow = {
        void apply(inout regthree_t value, out int<32> ret) {
            value.count = 0;
        }
    }; 
    ...
    RegisterAction<int<32>, bit<1>, bit<1>>(regfour) compare = {
        void apply(inout int<32> value, out bit<1> ret) {
            ret = 0;
            if((int<32>) meta.change < MEDIAN_THRESHOLD) {
                ret = 1;
            }
        }
    }; 
    ...
    apply {
        ...
        if(meta.flag == 0) { //we are entering a new window frame
            meta.change = newwindow.execute(0);
            meta.median = meta.median + meta.change;
        } 
        if(meta.flag == 1) {
            //we are in the first pane of a new window
            firstwindow.execute(0);
        }
        if(meta.flag == 2) {
            //we are in the middle of a window
            meta.count = midwindow.execute(0);
        }

        //5. Complete detection check from _regcompare_
