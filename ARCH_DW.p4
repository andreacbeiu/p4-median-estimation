    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    RegisterAction<bit<32>, bit<1>, bit<2>>(regtwo) updatetwo = {
        void apply(inout bit<32> curr, out bit<2> ret) {
            ret = 0;
            if(curr == 0) {
                //window = 0, first window case
                ret = 1;
                curr = curr + 1;
            } else {
                if(curr < WINDOW) {
                    //0 < window < threshold, middle of window case
                    curr = curr + 1;
                    ret = 2;
                } else {
                    //window >= threshold, we are at the end of the frame
                    curr = 0;
                    meta.flag = 0;
                }
            }
    ...
    RegisterAction<regthree_t, bit<1>, void>(regthree) firstwindow = {
        void apply(inout regthree_t value) {
            value.count = 0;
        }
    }; 
    ...
    Register<regfour_t, bit<1>>(1) regfour;
    RegisterAction<regfour_t, bit<1>, bit<1>>(regfour) compare = {
        void apply(inout regfour_t value, out bit<1> ret) {
            ret = 0;
            value.median_holder = (int<32>) meta.median + (int<32>) meta.change;
            if(value.median_holder < MEDIAN_THRESHOLD) {
                ret = 1;
            }
        }
    }; 

    RegisterAction<regfour_t, bit<1>, bit<1>>(regfour) save_median = {
        void apply(inout regfour_t value, out bit<1> ret) {
            ret = 0;
            value.median_holder = (int<32>) meta.median + (int<32>) meta.change;
        }
    ...
    apply {
        ...
        //3. comparison checker to write to meta.flag
        meta.flag = updatetwo.execute(0);


        //4. fork on which window stage we are in
        if(meta.flag == 0) { //we are starting a new window frame
            meta.c_val = newwindow.execute(0);  
            multiply_lookup.apply();
        } else if (meta.flag == 1) { //we are in the first window pane of a new frame, need to right median estimate to reg4
            firstwindow.execute(0);
            save_median.execute(0);
        } else if (meta.flag == 2) { //we are in the middle of a window frame, just need to increment/decrement
            midwindow.execute(0);
        }

        meta.median = meta.median + meta.change;
